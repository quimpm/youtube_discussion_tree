...
def __calculate_document_frequency(comments):
    DF = {}
...
def __in_doc_freq(word, doc_freq):
    if word in doc_freq:
        return doc_freq[word]
    else:
        return 0

def __preprocessing(comments):
    new_comments = []
...
def __gen_vector(tokens, total_vocab, doc_freq, N):
...
def __cosine_similarity(query, D, total_vocab, doc_freq, N):
    tokens = __preprocessing([query])[0]
    d_cosines = []
    query_vector = __gen_vector(tokens, total_vocab, doc_freq, N)
    for d in D:
...
def tf_idf_automatic_algorithm(replie, candidates):
    ...
    tf_idf = __calculate_tf_idf(clean_candidates, doc_freq, N, word_count)
